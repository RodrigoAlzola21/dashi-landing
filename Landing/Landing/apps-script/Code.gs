...
function validatePayload_(data) {
  ...
  if (!data.whatsapp) {
    throw new Error('ValidationError: whatsapp is required');
  }
  ...
  if (data.whatsapp) {
    if (!WHATSAPP_ALLOWED_REGEX.test(data.whatsapp)) {
      throw new Error('ValidationError: whatsapp invalid characters');
    }
    ...
    if (whatsappDigits.length < MIN_WHATSAPP_DIGITS || whatsappDigits.length > MAX_WHATSAPP_DIGITS) {
      throw new Error('ValidationError: whatsapp invalid digits length');
    }
  }
